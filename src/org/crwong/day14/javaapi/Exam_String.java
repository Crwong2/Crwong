...
public class Exam_String {
	public static void main(String[] args) {
		...
		// 문자열 포함되어 있는지 체크
		System.out.println("#이 있는가? : " + data1.contains("#"));   // contains  <-(" ") 이 있는지 체크
		
		// 문자열 연결
		//  C#,C++
		data1 = data1.concat(data2);  // concat <- data1 과 data2를 붙여주는 메소드
		System.out.println("연결된 문자열 : " + data1);
		
		// 문자열 공백 제거
		data1 = data1.trim();  // trim 은 공백 제거 메소드
		...
		for(int i = 0; i < words.length; i ++) {  // 1번 
			System.out.print(words[i] + " / ");
		}
		for(String word : words) {                // 2번  똑같음.
			System.out.print(word + " / ");
		}
		System.out.println();
		// 문자열 썰기2 - 인덱스로 썰어줌
//		data1 = data1.substring(4);         // 0~3까지 자름
		System.out.println("문자열 자르기 : " + data1);
		data1 = data1.substring(0, 4);      // 0~3까지 놔둠
